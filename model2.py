...
class TESTAM(nn.Module):
    ...
    def forward(self, input, gate_out = False):
        ...
        # h_future = [self.hidden_proj(input.permute(0,2,3,1))]
        B, N, T, _ = input.permute(0,2,3,1).size()
        h_future = [torch.ones((B, N, T, self.hidden_size)).to(input.device)]
        ...
        out = torch.zeros((B, N, T, 1)).view(-1,1).to(input.device)
