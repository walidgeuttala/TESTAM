...
class TemporalInformationEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        assert not torch.isnan(x_emb).any(), "NaN in embedding output"

        x_weight = self.linear(x_emb)
        assert not torch.isnan(x_weight).any(), "NaN in linear output"
        ...
        #assert not torch.isnan(x_linear).any(), "NaN in linear part"
        #assert not torch.isnan(x_act).any(), "NaN in activation part"
...
class TemporalModel(nn.Module):
    ...
    def forward(self, x, speed = None):
        ...
        #print("Initial embedding weights NaN check:", torch.isnan(self.embedding.embedding.weight).any())
        if torch.isnan(self.embedding.embedding.weight).any():
            nn.init.xavier_uniform_(self.embedding.embedding.weight)
        
        TIM = self.embedding(x)
        assert not torch.isnan(TIM).any(), "NaN in TIM embedding"
...
class TESTAM(nn.Module):
    ...
    def forward(self, input, gate_out = False):
        ...
        #cur_time_index = ((time_index * max_t) % max_t).long().clamp(0, self.max_time_index - 1)
        cur_time_index = ((time_index * max_t) % max_t).long().clamp(0, max_t - 1)
        next_time_index = ((time_index * max_t + time_index.size(-1)) % max_t).long().clamp(0, max_t - 1)
        #next_time_index = ((time_index * max_t + time_index.size(-1)) % max_t).long().clamp(0, self.max_time_index - 1)
